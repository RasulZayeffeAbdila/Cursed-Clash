...
const GAME_MODES={
  ...
  training:    {name:'Training',     winsNeeded:1, rounds:[999]}
};

// ── BINDING VOWS ──
const VOWS={
  overtime:     {name:'Overtime',            desc:'After 60 seconds, all your damage is ×1.5',                                          color:'#ff8800',icon:'⏱',short:'OVERTIME'},
  enchain:      {name:'Enchain',             desc:'Every 5s: 25% chance to blast 50dmg Shrine burst on opponent',                      color:'#cc1133',icon:'⛓',short:'ENCHAIN'},
  ...
  discharged:   {name:'Discharged Energy',   desc:'Start with a ready domain — no ult seal, reuse freely',                             color:'#ffcc00',icon:'⚡',short:'DISCHARGED'},
  overwhelming: {name:'Overwhelming Energy', desc:'When you manually activate your domain, you always win domain clashes — no seal',    color:'#aa44ff',icon:'💥',short:'OVERWHELMING'},
  adaptation:   {name:'Adaptation',          desc:'Every 100 damage you receive: +10% damage reduction (stacks, max 30%)',              color:'#44ff88',icon:'🛡',short:'ADAPT'},
  ...
  sixeyes:      {name:'Six Eyes',            desc:'-50% all cooldowns, but each skill costs 5 HP to use',                              color:'#55aaff',icon:'👁‍🗨',short:'SIX EYES'},
  domainVow:    {name:'Domain Vow',          desc:'Start with 25% less HP but always win domain clashes (useless without a domain)',    color:'#cc44ff',icon:'🌀',short:'DOMAIN VOW'},
  useless:      {name:'Useless Vow',         desc:'No buffs. No debuffs. Literally nothing.',                                          color:'#444444',icon:'❓',short:'USELESS'},
};
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  c.addEventListener('wheel',    e=>{if(state==='infopage')infoScroll=Math.max(0,infoScroll+e.deltaY*0.7);if(state==='historypage')histScroll=Math.max(0,histScroll+e.deltaY*0.7);if(state==='patchnotes')patchScroll=Math.max(0,patchScroll+e.deltaY*0.7);e.preventDefault();},{passive:false});
});