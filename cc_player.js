...
class Player{
  constructor(charName,sx,facing,num,altColor){
    ...
    this.jackpotBuff=null;          // null | 'odd' | 'even' — carries to next domain
    this._jackpotWinChance=0.25;    // base 25%; odd-buff raises to 40%
    this._jackpotSlotInterval=2500; // base 2500ms; even-buff lowers to 1500ms
    ...
    // Mahito: soul chip tracks permanent maxHp reduction
    this.soulChip=0;
    // Cursed Vessel state
    this.manjiCounterActive=false; this.manjiCounterTimer=0;
    this.cursedShield=0;
    // Binding Vow state
    this.vow=null;
  ...
  get ctrl(){return getCtrl(this.num);}
  isImmune(){return this.charName==='Heavenly Restriction';}
  cantClash(){return this.charName==='Heavenly Restriction'||this.charName==='Thunder God'||this.charName==='Star Rage'||this.charName==='Straw Doll'||this.charName==='Cursed Vessel'||!!(this.def&&this.def.cantDomainClash);}
  ...
  update(dt,opp){
    ...
    if(this.ultTimer>0){
      ...
      if(this.ultTimer<=0){
        ...
        if(this.inThunderUlt){
          ...
          SFX.burnout();
          spawnParticles(this.cx,this.cy,'#aa00ff',20);spawnParticles(this.cx,this.cy,'#fff',10);
    ...
    if(this.charName==='Fever Dreamer'){
      ...
      if(this.ultActive){
        ...
        if(this.jackpotSlotTimer<=0){
          this.jackpotSlotTimer=this._jackpotSlotInterval;
          this.jackpotSpinAnim=700;
          const win=Math.random()<this._jackpotWinChance;
          if(win){
            const d=Math.floor(Math.random()*10);
            this.jackpotDigits=[d,d,d];
            SFX.ultReady();
            spawnParticles(this.cx,this.cy,'#44ff88',15);spawnParticles(this.cx,this.cy,'#ffcc00',10);
            ...
            // Big background number reveal
            addFX({type:'jackpotBigNums',digits:[d,d,d],t:3800,x:W/2,y:H/2});
            // Determine next-domain buff from parity of winning digit
            const isOdd=d%2!==0;
            this.jackpotBuff=isOdd?'odd':'even';
            const buffMsg=isOdd?'★ ODD — NEXT DOMAIN: +40% WIN CHANCE':'★ EVEN — NEXT DOMAIN: SPEED ROLL (1.5s)';
            addFX({type:'bigText',x:W/2,y:H/2-54,text:buffMsg,color:isOdd?'#ff9944':'#44ffcc',t:3000});
            // Cancel domain, grant 14s standalone jackpot
    ...
    if(this.charName==='Cursed Vessel')spd=(this.ultActive?12.4:6.2); // ult doubles speed
    ...
    // P2 keyboard input is disabled in Player vs AI mode (AI drives p2)
    const inputAllowed=this.num===1||!aiEnabled;
    if(inputAllowed&&keys[ctrl.l]){this.vx=-spd;this.facing=-1;this.movedThisFrame=true;}
    else if(inputAllowed&&keys[ctrl.r]){this.vx=spd;this.facing=1;this.movedThisFrame=true;}
    else if(!inputAllowed)this.vx*=0.6;
    else this.vx*=0.6;
    const didJump=inputAllowed&&keys[ctrl.u]&&this.onGround;
    ...
    if(this.vow&&opp){
      ...
      // Overwhelming: player manually activates ult to guarantee clash win (handled in tryActivateDomain)
    ...
    // Cursed Vessel: Manji counter window decay
    if(this.manjiCounterActive&&this.manjiCounterTimer>0){
      this.manjiCounterTimer=Math.max(0,this.manjiCounterTimer-dt);
      if(this.manjiCounterTimer<=0){
        this.manjiCounterActive=false;
        addFX({type:'bigText',x:this.cx,y:this.y-42,text:'COUNTER MISSED',color:'#ff3377',t:700});
      }
    }
    // Cursed Vessel: shield cleared when ult ends
    if(this.charName==='Cursed Vessel'&&!this.ultActive&&this.cursedShield>0){
      this.cursedShield=0;
      addFX({type:'bigText',x:this.cx,y:this.y-42,text:'SHIELD GONE',color:'#ff3377',t:700});
    }
  } // end update()
  ...
  useSkill(slot,opp,forced=false){
    ...
    // No ult seal for discharged or overwhelming — they can reuse ult
    ...
    // Six Eyes vow: halve all cooldowns
    const cdMult=this.vow==='sixeyes'?0.5:1;
    this.cooldowns[slot]=this.def.skills[slot].cd*cdMult; this.atkAnim=440;
    // Six Eyes vow: -5hp per skill (not ult)
    if(this.vow==='sixeyes'&&slot<3&&this.hp>0){
      this.hp=Math.max(1,this.hp-5);
      addFX({type:'dmgNum',x:this.cx,y:this.y-18,val:5,color:'#55aaff',t:700});
      spawnParticles(this.cx,this.cy,'#55aaff',8);
    }
    ...
    else if(this.charName==='Cursed Vessel')this._cursedVesselSkill(slot,opp);
    else if(this.charName==='Idle Transfiguration')this._mahitoSkill(slot,opp);
  }
  ...
  _projSkill(slot,opp){
    ...
    switch(slot){
      case 0:{
        const MAX_STREAK=10;
        const streak=this.flashStreakCount||0;
        // Range grows a little each hit in the streak
        const range=160+streak*18;
        SFX.whoosh();
        const trails=Math.min(2+streak,10);
        for(let s=0;s<trails;s++)addFX({type:'afterimage',x:this.x+this.facing*s*38,y:this.y,color:col,t:460-s*40,a:1-s*0.09});
        const ox=this.x;
        this.x=Math.max(0,Math.min(W-PW,this.x+this.facing*range));
        addFX({type:'speedTrail',x1:ox+PW/2,x2:this.cx,y:this.cy,color:col,t:380});
        addFX({type:'flashBurst',x:this.cx,y:this.cy,color:col,t:300});
        spawnParticles(ox+PW/2,this.cy,col,10+streak*4);
        spawnParticles(this.cx,this.cy,col,10+streak*4);
        const minX=Math.min(ox,this.x-10),maxX=Math.max(ox+PW,this.x+PW+10);
        const hit=opp&&opp.x+PW>minX&&opp.x<maxX&&Math.abs(this.cy-opp.cy)<PH+20;
        if(hit){
          // ── HIT ──
          this.dealDamage(opp,10);
          if(streak>=MAX_STREAK){
            // Already at max — 10s CD, reset
            this.cooldowns[0]=7500;
            this.flashStreakCount=0;this.flashStreakTimer=0;
            addFX({type:'bigText',x:opp.cx,y:opp.y-68,text:'★ MAX FLASH ★',color:'#fff',t:1600});
            addFX({type:'screenFlash',color:'#fff',t:280,alpha:0.55,dur:280});
            addFX({type:'screenShake',t:400,mag:14});
            for(let i=0;i<5;i++)addFX({type:'burstRing',x:opp.cx,y:opp.cy,color:i%2===0?'#fff':col,t:380+i*40,r:28+i*32});
            spawnParticles(opp.cx,opp.cy,'#fff',12);spawnParticles(opp.cx,opp.cy,col,10);
          } else {
            // Hit while building — 1s window to chain next dash
            const newStreak=streak+1;
            this.flashStreakCount=newStreak;
            this.flashStreakTimer=2000; // 2 second window
            this.cooldowns[0]=0;      // no CD on hit
            addFX({type:'bigText',x:opp.cx,y:opp.y-42,text:'STEP ×'+newStreak,color:newStreak>=7?'#ffffff':col,t:700});
            if(newStreak>=4)addFX({type:'burstRing',x:this.cx,y:this.cy,color:col,t:300,r:38+newStreak*5});
            if(newStreak>=MAX_STREAK){
              addFX({type:'bigText',x:this.cx,y:this.cy-68,text:'⚡ MAX STREAK ×10 ⚡',color:'#ffffff',t:1600});
              addFX({type:'burstRing',x:this.cx,y:this.cy,color:'#fff',t:500,r:90});
              addFX({type:'screenFlash',color:col,t:200,alpha:0.28,dur:200});
              spawnParticles(this.cx,this.cy,'#fff',8);spawnParticles(this.cx,this.cy,col,8);
            }
          }
        } else {
          // ── MISS — streak breaks ──
          if(streak>=4){
            // Heavy punishment: 3s stun + 10s CD
            this.stunned=Math.max(this.stunned,3000);
            this.cooldowns[0]=10000;
            this.flashStreakCount=0;this.flashStreakTimer=0;
            addFX({type:'bigText',x:this.cx,y:this.cy-55,text:'STREAK BROKEN — 3s STUN',color:'#ff4422',t:1400});
            addFX({type:'screenShake',t:300,mag:9});
            spawnParticles(this.cx,this.cy,'#ff4422',25);
          } else if(streak>=1){
            // Soft break: just 10s CD, no stun
            this.cooldowns[0]=10000;
            this.flashStreakCount=0;this.flashStreakTimer=0;
            addFX({type:'bigText',x:this.cx,y:this.cy-44,text:'STREAK MISSED — 10s CD',color:'#ff8844',t:1100});
            spawnParticles(this.cx,this.cy,'#ff8844',15);
          } else {
            // No streak, plain miss — 5s CD
  ...
  _shrineSkill(slot,opp){
    ...
    switch(slot){
      case 0:{SFX.slashHeavy();const reach=160;addFX({type:'arcSlash',x:this.cx,y:this.cy,facing:this.facing,color:col,t:450,rad:reach});spawnParticles(this.cx+this.facing*80,this.cy,col,45);if(this._dist(opp)<reach+25){SFX.slash();this.dealDamage(opp,32);addFX({type:'slashMark',x:opp.cx,y:opp.cy,color:col,t:700});addFX({type:'bigText',x:opp.cx,y:opp.y-30,text:'CLEAVE',color:col,t:550});}break;}
  ...
  _limitlessSkill(slot,opp){
    switch(slot){
      ...
      case 3:{SFX.domain();this.ult=0;this.domainActive=true;this.domainDmgTotal=0;this.ultActive=true;this.ultTimer=5500;
        if(!opp.isImmune()){opp.infiniteVoidActive=true;opp.stunned=5000;addFX({type:'infiniteVoidBg',t:5500});addFX({type:'domainText',text:'Domain Expansion',sub:'INFINITE VOID',color:'#ffffff',t:3500});addFX({type:'bigText',x:opp.cx,y:opp.y-20,text:'OVERWHELMED',color:'#cc88ff',t:4500});spawnParticles(W/2,H/2,'#fff',200);spawnParticles(W/2,H/2,'#aa88ff',150);const self2=this,op=opp;setTimeout(()=>{op.infiniteVoidActive=false;op.stunned=0;self2.domainActive=false;},5000);}
  ...
  _thunderUltSkill(slot,opp){
    ...
    switch(slot){
      ...
      case 1:{this.thunderUltCDs[1]=4000;this.atkAnim=550;SFX.godslayer();const gsFX={type:'godslayer',x:opp.cx,y:0,t:700,triggered:false};addFX(gsFX);spawnParticles(opp.cx,H/2,'#aa44ff',60);spawnParticles(opp.cx,FLOOR,'#fff',40);const self=this;setTimeout(()=>{if(self._dist(opp)<350){self.dealDamage(opp,65);opp.stunned=1000;gsFX.triggered=true;addFX({type:'bigText',x:opp.cx,y:opp.y-30,text:'GODSLAYER',color:'#cc44ff',t:900});}},450);break;}
  ...
  _feverSkill(slot,opp){
    ...
    switch(slot){
      ...
      case 2:{ // Cursed Door
        ...
        effects.push({type:'cursedDoor',x:dx,y:FLOOR-dh,w:dw,h:dh,ownerId:this.num,oppId:opp.num,dmg:jp2?38:24,dcol:doorCol,t:5000,trapped:false,trapTimer:0,launched:false,jp:jp2});
      ...
      case 3:{ // Idle Death Gamble — domain
        ...
        this.jackpotActive=false;this.jackpotTimer=0;
        // Apply any carry-over jackpot buff from previous win
        this._jackpotWinChance=this.jackpotBuff==='odd'?0.40:0.25;
        this._jackpotSlotInterval=this.jackpotBuff==='even'?1500:2500;
        if(this.jackpotBuff){
          const buffLabel=this.jackpotBuff==='odd'?'★ ODD BUFF: 40% WIN CHANCE ACTIVE':'★ EVEN BUFF: 1.5s ROLL ACTIVE';
          addFX({type:'bigText',x:W/2,y:H/2+40,text:buffLabel,color:this.jackpotBuff==='odd'?'#ff9944':'#44ffcc',t:2800});
          addFX({type:'screenFlash',color:this.jackpotBuff==='odd'?'#ff4400':'#00ffcc',t:350,alpha:0.22,dur:350});
        }
        this.jackpotBuff=null; // consumed — next win will set a fresh buff
        this.jackpotSlotTimer=this._jackpotSlotInterval;
  ...
  dealDamage(opp,amount){
    ...
    // Soul Isomer — active isomer absorbs ONE incoming hit for Mahito
    if(opp.charName==='Idle Transfiguration'){
      const _iso=effects.find(e=>e.type==='soulIsomer'&&e.owner===opp&&!e.hit&&e.t>0);
      if(_iso){_iso.hit=true;_iso.t=0;SFX.block();spawnParticles(_iso.x,_iso.y,opp.color,20);addFX({type:'burstRing',x:_iso.x,y:_iso.y,color:opp.color,t:400,r:55});addFX({type:'bigText',x:opp.cx,y:opp.y-44,text:'ISOMER SHIELDED',color:opp.color,t:950});return;}
    }
    // Manji Kick counter — intercept hit, stun attacker, reflect 75%
    if(opp.manjiCounterActive&&!opp.isImmune()){
      opp.manjiCounterActive=false;opp.manjiCounterTimer=0;
      const reflect=Math.round(amount*0.75);
      this.stunned=Math.max(this.stunned,1200);this.hp=Math.max(0,this.hp-reflect);this.hurtAnim=400;
      SFX.bigHit();
      addFX({type:'screenFlash',color:'#ff3377',t:200,alpha:0.45,dur:200});
      addFX({type:'screenShake',t:280,mag:12});
      addFX({type:'burstRing',x:opp.cx,y:opp.cy,color:'#ff3377',t:400,r:60});
      spawnParticles(opp.cx,opp.cy,'#ff3377',40);spawnParticles(opp.cx,opp.cy,'#fff',20);
      addFX({type:'bigText',x:this.cx,y:this.y-55,text:'MANJI COUNTER!',color:'#ff3377',t:1400});
      addFX({type:'dmgNum',x:this.cx,y:this.y-5,val:reflect,color:'#ff3377',t:1100});
      if(!this.ultActive)opp.ult=Math.min(MAX_ULT,opp.ult+reflect*0.5);
      return;
    }
    ...
    // True damage: Mahito's domain bypasses all OPPONENT-side DR, shields, and adaptation
    const _trueDmg=this.charName==='Idle Transfiguration'&&this.ultActive;
    // Adaptation damage reduction
    if(!_trueDmg&&opp.vow==='adaptation'&&opp.vowData.adaptStacks>0){
      const red=Math.min(0.3,opp.vowData.adaptStacks*0.1);
      dmg=Math.max(1,Math.round(dmg*(1-red)));
    }
    // Cursed Nullification — permanent 20% DR
    if(!_trueDmg&&opp.vow==='cursednull')dmg=Math.max(1,Math.round(dmg*0.8));
    // Cursed Vessel ult — 20% DR
    if(!_trueDmg&&opp.charName==='Cursed Vessel'&&opp.ultActive)dmg=Math.max(1,Math.round(dmg*0.8));
    // Cursed Vessel shield — absorb damage before HP
    if(!_trueDmg&&opp.charName==='Cursed Vessel'&&opp.cursedShield>0){
      const absorbed=Math.min(opp.cursedShield,dmg);
      opp.cursedShield-=absorbed;dmg-=absorbed;
      if(absorbed>0){spawnParticles(opp.cx,opp.cy,'#ff3377',12);addFX({type:'dmgNum',x:opp.cx,y:opp.y-5,val:absorbed,color:'#ff3377',t:900});}
      if(opp.cursedShield<=0){addFX({type:'bigText',x:opp.cx,y:opp.y-55,text:'SHIELD BROKEN',color:'#ff3377',t:1100});addFX({type:'burstRing',x:opp.cx,y:opp.cy,color:'#ff3377',t:400,r:62});spawnParticles(opp.cx,opp.cy,'#ff3377',40);}
      if(dmg<=0)return;
    }
    if(this.ultActive&&this.charName==='Heavenly Restriction')dmg=Math.round(dmg*3);
    opp.hp=Math.max(0,opp.hp-dmg); opp.hurtAnim=300;
    // Hit freeze — big hits (≥30dmg) freeze physics for ~85ms like a real fighting game
    if(dmg>=30)hitFreezeTimer=Math.max(hitFreezeTimer,85);
    if(!this.ultActive)this.ult=Math.min(MAX_ULT,this.ult+dmg*0.65);
    // Defender gains ult from being hit, UNLESS attacker's ult is active (prevents "P2 gets ult from P1's ult hits")
    if(!this.ultActive&&!opp.ultActive)opp.ult=Math.min(MAX_ULT,opp.ult+dmg*0.3);
    ...
    if(opp.vow==='adaptation'){
      ...
      while(opp.vowData.adaptAcc>=100&&opp.vowData.adaptStacks<3){
    ...
    switch(slot){
      case 0:{ // Mass Burst — full imaginary mass slam, 45dmg, 240px range
        ...
        if(this._dist(opp)<240){
          this.dealDamage(opp,25);
          opp.vx=this.facing*28;opp.vy=-13;
          if(this.ultActive)opp.stunned=Math.max(opp.stunned,500);
      ...
      case 1:{ // Imaginary Mass — windup, then fire fast straight dense shot, 40dmg
        ...
        setTimeout(()=>{
          ...
          const orb={type:'massOrb',x:fx,y:self2.cy,vx:self2.facing*22,vy:0,
            owner:self2,opp,dmg:28,color:col,t:900,r:28,hit:false,trail:[],straight:true,ultStun:self2.ultActive};
          effects.push(orb);
      ...
      case 2:{ // Mass Augment — charge up, powered devastating slam, 55dmg
        ...
        setTimeout(()=>{
          ...
          if(self._dist(opp)<180){
            self.dealDamage(opp,40);
            opp.vx=self.facing*36;opp.vy=-16;
            if(self.ultActive)opp.stunned=Math.max(opp.stunned,500);
            addFX({type:'screenShake',t:600,mag:20});
    ...
    if(marked){
      ...
      switch(slot){
        ...
        case 2:{ // Divergence — 65dmg AoE explosion, consumes mark
          ...
          consumeMark();break;}
        case 3:{ // Black Flash — far dash, 75dmg + THE ZONE, consumes mark
          // ult is also zeroed here defensively in case called outside tryActivateDomain
          ...
          setTimeout(()=>{
            ...
            if(self._dist(opp)<200){
              ...
              self.dealDamage(opp,75);opp.vx=self.facing*40;opp.vy=-20;
      ...
      switch(slot){
        case 0:{ // Hammer Strike — 34dmg, heavy stagger
          ...
          if(this._dist(opp)<160){
            this.dealDamage(opp,22);opp.vx=this.facing*18;opp.vy=-11;
        ...
        case 1:{ // Nail Barrage — 3 nails, 18dmg each
          ...
          [-0.2,0,0.2].forEach((ang,i)=>setTimeout(()=>{
            ...
            effects.push({type:'nail',x:nx,y:ny+ang*60,vx:this.facing*15,vy:ang*9,
              owner:this,opp,dmg:12,color:col,t:1600,hit:false,trail:[]});
            addFX({type:'burstRing',x:nx,y:ny+ang*60,color:col,t:180,r:18});
          },i*80));
          spawnParticles(this.facing>0?this.x+PW:this.x,ny,col,14);
          break;}
        case 2:{ // Resonance — fires a cursed nail projectile; on hit: 28dmg + MARK
          SFX.vortex();SFX.whoosh();this.atkAnim=280;
          spawnParticles(this.cx,this.cy,col,18);
          addFX({type:'burstRing',x:this.cx,y:this.cy,color:col,t:260,r:36});
          const nx2=this.facing>0?this.x+PW+4:this.x-4;
          effects.push({type:'resonanceOrb',x:nx2,y:this.cy-10,
            vx:this.facing*14,vy:-2,owner:self,opp,dmg:18,color:col,t:1800,hit:false,trail:[],tracking:true,trackTimer:900});
          break;}
  ...
  _cursedVesselSkill(slot,opp){
    const col=this.color,self=this;
    const inUlt=this.ultActive;
    switch(slot){
      case 0:{ // Divergent Fist / Black Flash
        const blackFlashChance=inUlt?0.50:0.25;
        const isBlackFlash=Math.random()<blackFlashChance;
        const range=140;
        if(isBlackFlash)SFX.blackFlash(); else SFX.swingHeavy();
        this.atkAnim=500;
        // Wind-up flash
        addFX({type:'burstRing',x:this.cx,y:this.cy,color:col,t:280,r:40});
        spawnParticles(this.facing>0?this.x+PW:this.x,this.cy,col,20);
        if(isBlackFlash){
          // BLACK FLASH — build-up then strike
          addFX({type:'screenFlash',color:'#000',t:160,alpha:0.6,dur:160});
          addFX({type:'bigText',x:this.cx,y:this.cy-70,text:'⚡ BLACK FLASH ⚡',color:'#fff',t:1100});
          spawnParticles(this.cx,this.cy,'#000',30);spawnParticles(this.cx,this.cy,col,30);
          setTimeout(()=>{
            if(!opp)return;
            if(self._dist(opp)<range){
              self.dealDamage(opp,50);
              opp.stunned=Math.max(opp.stunned,500);
              opp.vx=self.facing*20;opp.vy=-9;
              // Black lightning shockwave rings
              for(let i=0;i<5;i++)addFX({type:'burstRing',x:opp.cx,y:opp.cy,color:i%2===0?'#000':'#fff',t:380+i*40,r:24+i*28});
              addFX({type:'screenShake',t:500,mag:18});
              addFX({type:'screenFlash',color:'#000',t:280,alpha:0.85,dur:280});
              addFX({type:'screenFlash',color:col,t:260,alpha:0.55,dur:260});
              spawnParticles(opp.cx,opp.cy,'#000',60);spawnParticles(opp.cx,opp.cy,'#fff',40);spawnParticles(opp.cx,opp.cy,col,40);
              addFX({type:'bigText',x:opp.cx,y:opp.y-60,text:'✦ BLACK FLASH ✦',color:'#fff',t:1400});
              // Zone buff
              self.zoneActive=true;self.zoneTimer=5000;
              addFX({type:'bigText',x:self.cx,y:self.cy-92,text:'⚡ THE ZONE ⚡',color:'#ffaa00',t:1600});
              addFX({type:'screenFlash',color:'#ffaa00',t:220,alpha:0.35,dur:220});
              spawnParticles(self.cx,self.cy,'#ffaa00',40);
              // CD = 0 on hit
              self.cooldowns[0]=0;
            } else {
              // Missed — normal CD applies (already set)
              addFX({type:'bigText',x:self.cx,y:self.cy-50,text:'BLACK FLASH MISSED',color:'#ff4422',t:900});
              spawnParticles(self.cx,self.cy,'#ff4422',15);
            }
          },180);
        } else {
          // DIVERGENT FIST — two-hit
          if(this._dist(opp)<range){
            this.dealDamage(opp,15);opp.vx=this.facing*12;opp.vy=-7;
            addFX({type:'bigText',x:opp.cx,y:opp.y-42,text:'DIVERGENT FIST',color:col,t:800});
            addFX({type:'slashLine',x1:this.cx,y1:this.cy,x2:opp.cx,y2:opp.cy,color:col,t:220});
            spawnParticles(opp.cx,opp.cy,col,22);
            // Delayed cursed shock +15dmg
            setTimeout(()=>{
              if(!opp||opp.hp<=0)return;
              self.dealDamage(opp,10);
              addFX({type:'burstRing',x:opp.cx,y:opp.cy,color:col,t:350,r:44});
              addFX({type:'burstRing',x:opp.cx,y:opp.cy,color:'#000',t:300,r:26});
              addFX({type:'screenFlash',color:col,t:150,alpha:0.25,dur:150});
              spawnParticles(opp.cx,opp.cy,col,18);spawnParticles(opp.cx,opp.cy,'#000',10);
              addFX({type:'bigText',x:opp.cx,y:opp.y-58,text:'CURSED SHOCK',color:'#fff',t:800});
            },200);
          }
        }
        break;}

      case 1:{ // Manji Kick — counter window
        SFX.guard();this.atkAnim=300;
        this.manjiCounterActive=true;this.manjiCounterTimer=1000;
        addFX({type:'hexBarrier',owner:this,t:1000});
        spawnParticles(this.cx,this.cy,col,30);spawnParticles(this.cx,this.cy,'#fff',15);
        addFX({type:'burstRing',x:this.cx,y:this.cy,color:col,t:380,r:55});
        addFX({type:'bigText',x:this.cx,y:this.y-52,text:'MANJI KICK — COUNTER',color:col,t:900});
        addFX({type:'screenFlash',color:col,t:160,alpha:0.18,dur:160});
        break;}

      case 2:{ // Cursed Rush — dash + combo
        SFX.whoosh();this.atkAnim=700;
        const ox=this.cx;
        // Dash to opponent
        if(opp)this.x=Math.max(0,Math.min(W-PW,opp.cx+(this.facing>0?-PW-18:18)));
        addFX({type:'speedTrail',x1:ox,x2:this.cx,y:this.cy,color:col,t:380});
        spawnParticles(this.cx,this.cy,col,25);
        const hits=4+Math.floor(Math.random()*2); // 4 or 5
        for(let hi=0;hi<hits;hi++){
          setTimeout(()=>{
            if(!opp||opp.hp<=0)return;
            if(self._dist(opp)<130){
              self.dealDamage(opp,5);
              opp.vx=self.facing*5;
              const hx=opp.cx+(Math.random()-.5)*30,hy=opp.cy+(Math.random()-.5)*30;
              addFX({type:'slashLine',x1:self.cx,y1:self.cy,x2:hx,y2:hy,color:col,t:200});
              spawnParticles(hx,hy,col,6);
              if(hi===0)addFX({type:'bigText',x:opp.cx,y:opp.y-40,text:'CURSED RUSH',color:col,t:700});
            }
          },hi*100);
        }
        // Final heavy punch
        setTimeout(()=>{
          if(!opp||opp.hp<=0)return;
          if(self._dist(opp)<140){
            self.dealDamage(opp,10);opp.vx=self.facing*26;opp.vy=-13;
            addFX({type:'crushRing',x:opp.cx,y:opp.cy,color:col,t:420});
            addFX({type:'screenShake',t:280,mag:10});
            addFX({type:'burstRing',x:opp.cx,y:opp.cy,color:col,t:350,r:58});
            spawnParticles(opp.cx,opp.cy,col,30);spawnParticles(opp.cx,opp.cy,'#fff',15);
            addFX({type:'bigText',x:opp.cx,y:opp.y-56,text:'FINAL BLOW',color:'#fff',t:950});
          }
        },hits*100+120);
        break;}

      case 3:{ // Cursed Fingers — 10s ult
        SFX.domain();this.ult=0;this.ultActive=true;this.ultTimer=10000;
        this.cursedShield=100;
        addFX({type:'domainText',text:'Cursed Fingers',sub:"SUKUNA'S POWER AWAKENS",color:col,t:3000});
        addFX({type:'screenFlash',color:'#000',t:300,alpha:0.7,dur:300});
        addFX({type:'screenFlash',color:col,t:260,alpha:0.55,dur:260});
        addFX({type:'screenShake',t:500,mag:14});
        spawnParticles(this.cx,this.cy,'#000',80);spawnParticles(this.cx,this.cy,col,80);spawnParticles(this.cx,this.cy,'#fff',40);
        for(let i=0;i<6;i++)addFX({type:'burstRing',x:this.cx,y:this.cy,color:i%2===0?col:'#000',t:400+i*50,r:30+i*36});
        addFX({type:'bigText',x:this.cx,y:this.cy-80,text:'⚫ CURSED FINGERS ⚫',color:'#fff',t:1800});
        break;}
    }
  }


  _mahitoSkill(slot,opp){
    const col=this.color,self=this;
    switch(slot){
      case 0:{ // Soul Punch — 20dmg + permanent maxHp reduction
        SFX.crush();this.atkAnim=400;
        const ix=this.facing>0?this.x+PW+10:this.x-10;
        addFX({type:'burstRing',x:ix,y:this.cy,color:col,t:320,r:50});
        for(let i=0;i<6;i++){const a=i/6*Math.PI*2;addFX({type:'slashLine',x1:ix,y1:this.cy,x2:ix+Math.cos(a)*60,y2:this.cy+Math.sin(a)*45,color:col,t:260});}
        spawnParticles(ix,this.cy,col,30);
        if(this._dist(opp)<130){
          this.dealDamage(opp,20);
          const reduction=12;
          opp.maxHp=Math.max(50,opp.maxHp-reduction);
          opp.hp=Math.min(opp.hp,opp.maxHp);
          addFX({type:'bigText',x:opp.cx,y:opp.y-48,text:'SOUL DEGRADED  MAX-'+reduction,color:col,t:1300});
          addFX({type:'screenFlash',color:col,t:180,alpha:0.22,dur:180});
          spawnParticles(opp.cx,opp.cy,col,25);spawnParticles(opp.cx,opp.cy,'#fff',10);
        }
        break;}
      case 1:{ // Soul Isomer — tracking humanoid + absorbs 1 hit for owner
        SFX.vortex();this.atkAnim=350;
        const bx=this.facing>0?this.x+PW+6:this.x-6;
        effects.push({type:'soulIsomer',x:bx,y:this.cy-10,vx:this.facing*8,vy:-2,
          owner:this,opp,dmg:18,color:col,t:2800,hit:false,trail:[],tracking:true,trackTimer:1800});
        addFX({type:'burstRing',x:bx,y:this.cy,color:col,t:360,r:40});
        addFX({type:'bigText',x:bx,y:this.cy-56,text:'SOUL ISOMER',color:col,t:700});
        spawnParticles(bx,this.cy,col,25);
        break;}
      case 2:{ // Body Geometry — wide burst + root
        SFX.seismic();this.atkAnim=500;
        const range=200;
        addFX({type:'arcSlash',x:this.cx,y:this.cy,facing:this.facing,color:col,t:380,rad:range});
        for(let i=0;i<4;i++)addFX({type:'burstRing',x:this.cx,y:this.cy,color:i%2===0?col:'#fff',t:380+i*40,r:28+i*44});
        addFX({type:'screenFlash',color:col,t:180,alpha:0.26,dur:180});
        addFX({type:'groundCrack',x:this.cx,y:FLOOR,color:col,t:700,big:false});
        spawnParticles(this.cx,this.cy,col,50);
        addFX({type:'bigText',x:this.cx,y:this.cy-58,text:'BODY GEOMETRY',color:col,t:800});
        if(this._dist(opp)<range+30){
          this.dealDamage(opp,26);
          opp.stunned=Math.max(opp.stunned,700);
          opp.vx*=0.08;opp.vy*=0.08;
          addFX({type:'bigText',x:opp.cx,y:opp.y-44,text:'WARPED',color:'#fff',t:900});
          addFX({type:'screenShake',t:280,mag:10});
        }
        break;}
      case 3:{ // Self-Embodiment of Perfection — 8s true damage domain
        SFX.domain();this.ult=0;this.domainActive=true;this.domainDmgTotal=0;
        this.ultActive=true;this.ultTimer=8000;
        spawnParticles(this.cx,this.cy,col,120);spawnParticles(this.cx,this.cy,'#fff',60);spawnParticles(this.cx,this.cy,'#888',40);
        addFX({type:'mahitoBg',owner:this,t:8000});
        addFX({type:'domainText',text:'Domain Expansion',sub:'SELF-EMBODIMENT OF PERFECTION',color:col,t:3000});
        addFX({type:'screenFlash',color:'#fff',t:400,alpha:0.85,dur:400});
        addFX({type:'screenFlash',color:col,t:350,alpha:0.55,dur:350});
        addFX({type:'bigText',x:W/2,y:H/2+54,text:'⚠ TRUE DAMAGE — ALL DR BYPASSED ⚠',color:'#fff',t:3200});
        break;}
    }
  }
  ...
  draw(){
    ...
    let glowAmt=tgGlowing?32:(this.inThunderUlt?28:(this.ultActive?26:(this.atkAnim>0?14:(this.stagnationReady?16:0))));
    let glowCol=tgGlowing?'#00aaff':(this.inThunderUlt?'#aa00ff':(this.stagnationReady?'#99aaff':col));
    if(this.jackpotActive){glowAmt=40;glowCol='#00ff55';}
    ...
    // Cursed Vessel: Manji counter indicator
    if(this.manjiCounterActive){
      const _t=Date.now()*0.003;
      ctx.save();ctx.globalAlpha=0.5+0.3*Math.sin(_t*14);
      ctx.strokeStyle='#ff3377';ctx.lineWidth=3;ctx.shadowColor='#ff3377';ctx.shadowBlur=20;
      ctx.setLineDash([4,3]);ctx.strokeRect(this.x-7,this.y-7,PW+14,PH+14);ctx.setLineDash([]);
      ctx.globalAlpha=0.85+0.15*Math.sin(_t*10);
      txt('COUNTER',this.cx,this.y-16,8,'#ff3377');
      ctx.restore();
    }
    // Cursed Vessel: cursed energy shield bar + aura
    if(this.cursedShield>0){
      const _t=Date.now()*0.003;
      // Shield outer ring
      ctx.save();ctx.globalAlpha=0.35+0.2*Math.sin(_t*8);
      ctx.strokeStyle='#ff3377';ctx.lineWidth=4;ctx.shadowColor='#ff3377';ctx.shadowBlur=24;
      ctx.beginPath();ctx.ellipse(this.cx,this.cy,PW*0.75,PH*0.85,0,0,Math.PI*2);ctx.stroke();
      ctx.restore();
    }
    // Cursed Vessel ult — Sukuna-style tattoo aura
    if(this.charName==='Cursed Vessel'&&this.ultActive){
      const _t=Date.now()*0.001;
      ctx.save();
      // Swirling dark+pink tattoo lines
      for(let i=0;i<8;i++){
        const ang=i/8*Math.PI*2+_t*2.5;
        const r1=38,r2=62+Math.sin(_t*6+i)*12;
        ctx.globalAlpha=0.65+0.25*Math.sin(_t*4+i);
        ctx.strokeStyle=i%2===0?'#ff3377':'#000';ctx.lineWidth=2.2;
        ctx.shadowColor=i%2===0?'#ff0055':'transparent';ctx.shadowBlur=i%2===0?10:0;
        ctx.beginPath();ctx.moveTo(this.cx+Math.cos(ang)*r1,this.cy+Math.sin(ang)*r1*0.65);
        ctx.lineTo(this.cx+Math.cos(ang+0.4)*r2,this.cy+Math.sin(ang+0.4)*r2*0.65);ctx.stroke();
      }
      // Black + pink inner pulse
      ctx.globalAlpha=0.18+0.1*Math.sin(_t*9);
      const sg=ctx.createRadialGradient(this.cx,this.cy,5,this.cx,this.cy,70);
      sg.addColorStop(0,'#ff0055');sg.addColorStop(0.5,'#440011');sg.addColorStop(1,'rgba(0,0,0,0)');
      ctx.fillStyle=sg;ctx.beginPath();ctx.ellipse(this.cx,this.cy,70,56,0,0,Math.PI*2);ctx.fill();
      ctx.globalAlpha=0.8+0.2*Math.sin(_t*10);
      txt('CURSED FINGERS',this.cx,this.y+PH+14,7,'#ff3377');
      ctx.restore();
    }
    // Thunder ult purple aura
    if(this.inThunderUlt){
      const t2=Date.now()*0.001;
      ctx.globalAlpha=0.7;ctx.strokeStyle='#aa00ff';ctx.lineWidth=2;ctx.shadowColor='#aa00ff';ctx.shadowBlur=12;
      ctx.beginPath();for(let i=0;i<6;i++){const ang=i/6*Math.PI*2+t2*3,r1=40,r2=65+Math.sin(t2*8+i)*15;ctx.moveTo(this.cx+Math.cos(ang)*r1,this.cy+Math.sin(ang)*r1);ctx.lineTo(this.cx+Math.cos(ang+0.35)*r2,this.cy+Math.sin(ang+0.35)*r2);}ctx.stroke();
      ctx.globalAlpha=1;
    }
    ...
    if(this.chargeTimer>0){
      ...
      ctx.globalAlpha=0.65;ctx.strokeStyle=this.chargeOwnerColor;ctx.lineWidth=2;ctx.shadowColor=this.chargeOwnerColor;ctx.shadowBlur=10;
      ctx.beginPath();for(let i=0;i<4;i++){const ang=i*Math.PI/2+t2,r=48+Math.sin(t2*4+i)*10;ctx.moveTo(this.cx+Math.cos(ang)*r,this.cy+Math.sin(ang)*r);ctx.lineTo(this.cx+Math.cos(ang+0.5)*(r+14),this.cy+Math.sin(ang+0.5)*(r+14));}ctx.stroke();
      ctx.globalAlpha=1;
    }
    ...
    // Idle Transfiguration: show accumulated soulChip debuff above opponent head
    if(this.charName==='Idle Transfiguration'&&this.ultActive){
      const _t2=Date.now()*0.001;
      ctx.save();ctx.globalAlpha=0.55+0.25*Math.sin(_t2*10);
      ctx.strokeStyle=col;ctx.lineWidth=2;ctx.shadowColor=col;ctx.shadowBlur=18;
      ctx.setLineDash([3,4]);ctx.strokeRect(this.x-8,this.y-8,PW+16,PH+16);ctx.setLineDash([]);
      ctx.globalAlpha=0.85+0.15*Math.sin(_t2*8);
      txt('TRUE DMG',this.cx,this.y-18,7,col);
      ctx.restore();
    }
    ctx.globalAlpha=1;ctx.shadowBlur=0;
    // Player badge
