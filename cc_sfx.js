...
const SFX=(()=>{
  ...
  return{
    ...
    // ── UI / menu sounds ──
    hover(){const c=g();if(!c)return;const o=c.createOscillator(),gn=c.createGain();o.type='sine';o.frequency.value=720;gn.gain.setValueAtTime(0.04,c.currentTime);gn.gain.exponentialRampToValueAtTime(0.0001,c.currentTime+0.04);o.connect(gn);gn.connect(c.destination);o.start();o.stop(c.currentTime+0.06);},
    uiClick(){noise(0.04,0.14,3500);tone(420,'square',0.06,0.22);},
    uiBack(){sweep(360,180,'sine',0.09,0.18);noise(0.05,0.08,4000);},
    uiConfirm(){[0,70,140].forEach((d,i)=>setTimeout(()=>tone([440,554,660][i],'triangle',0.11,0.26),d));},
    charBrowse(){noise(0.05,0.1,7000);tone(300,'sine',0.07,0.14);},
    // ── Character ability sounds ──
    blackFlash(){const c=g();if(!c)return;
      // Big distortion sweep + sub boom + crackle
      noise(0.3,0.7,180);
      sweep(2200,55,'sawtooth',0.35,0.75);
      tone(45,'sine',0.5,0.8);
      setTimeout(()=>{sweep(1800,40,'sawtooth',0.25,0.45);noise(0.18,0.5,300);},80);
    },
    nail(){noise(0.07,0.55,380);tone(260,'square',0.05,0.38);sweep(800,320,'sawtooth',0.06,0.25);},
    swingHeavy(){noise(0.14,0.32,5500);sweep(520,140,'sawtooth',0.14,0.28);},
    gravPull(){noise(0.5,0.35,800);sweep(220,44,'sine',0.6,0.42);tone(66,'sine',0.5,0.3);}
  };
})();