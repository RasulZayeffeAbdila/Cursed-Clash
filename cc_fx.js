...
function spawnParticles(x,y,color,n){const cap=Math.min(n,10);if(particles.length>=28)return;const room=28-particles.length;const actual=Math.min(cap,room);for(let i=0;i<actual;i++){const a=Math.random()*Math.PI*2,s=Math.random()*4.5+1;particles.push({x,y,vx:Math.cos(a)*s,vy:Math.sin(a)*s-2,color,alpha:1,r:Math.random()*3.5+1,life:260+Math.random()*320});}}
...
function updateFX(dt){
  ...
  // Hard cap: only trim cosmetic/transient effects — NEVER trim persistent functional effects
  // (domain bgs, projectiles, auras, traps). Star Rage skills generate ~13 effects at once
  // which used to push domain backgrounds off the array, cancelling domains visually + functionally.
  const _TRANSIENT=new Set(['burstRing','screenFlash','screenShake','bigText','slashLine','slashMark',
    'speedTrail','afterimage','flashBurst','groundCrack','seismicWave','dmgNum','redExplosion',
    'hollowImpact','clockSpiral','timeStopSplash','blockSpark','jumpTrail','crossImpact',
    'staticRushTrail','amberDischarge','arcSlash','jackpotWin','jackpotBigNums','domainText',
    'crushRing','crushSpin']);
  const _TRANSIENT_CAP=20;
  if(effects.length>0){
    const _pinned=effects.filter(e=>!_TRANSIENT.has(e.type));
    const _transient=effects.filter(e=>_TRANSIENT.has(e.type));
    if(_transient.length>_TRANSIENT_CAP)_transient.splice(0,_transient.length-_TRANSIENT_CAP);
    effects=[..._pinned,..._transient];
  }
  ...
  effects=effects.filter(e=>{
    ...
    if(e.type==='bolt'){
      e.x+=e.vx;e.y+=e.vy;
      if(e.trail){e.trail.push({x:e.x,y:e.y});if(e.trail.length>8)e.trail.shift();}
    ...
    if(e.type==='redOrb'){e.x+=e.vx;e.y+=e.vy;if(e.trail){e.trail.push({x:e.x,y:e.y});if(e.trail.length>10)e.trail.shift();}if(!e.hit&&e.opp){const d=Math.hypot(e.x-e.opp.cx,e.y-e.opp.cy);if(d<55){e.owner.dealDamage(e.opp,e.dmg);e.opp.vx=e.vx*2.8;e.opp.vy=-13;spawnParticles(e.x,e.y,'#ff4400',12);addFX({type:'redExplosion',x:e.x,y:e.y,t:600});e.hit=true;return false;}}if(e.x<-60||e.x>W+60)return false;}
    if(e.type==='hollowPurple'){e.x+=e.vx;e.y+=e.vy;if(!e.hit&&e.opp){const d=Math.hypot(e.x-e.opp.cx,e.y-e.opp.cy);if(d<90){e.owner.dealDamage(e.opp,e.dmg);e.opp.vx=e.vx*2.2;e.opp.vy=-8;spawnParticles(e.x,e.y,'#aa44ff',15);spawnParticles(e.x,e.y,'#fff',8);addFX({type:'hollowImpact',x:e.x,y:e.y,t:900});addFX({type:'screenFlash',color:'#aa44ff',t:300,alpha:0.5,dur:300});e.hit=true;return false;}}if(e.x<-150||e.x>W+150)return false;}
    if(e.type==='voltageCage'&&!e.triggered&&e.opp){const o=e.opp;if(o.x<e.x+e.w&&o.x+PW>e.x&&o.y<e.y+e.h&&o.y+PH>e.y){e.owner.dealDamage(o,32);o.stunned=Math.max(o.stunned,1500);spawnParticles(o.cx,o.cy,'#00ccff',12);addFX({type:'bigText',x:o.cx,y:o.y-30,text:'SHOCKED!',color:'#00ccff',t:900});addFX({type:'burstRing',x:o.cx,y:o.cy,color:'#00aaff',t:400,r:70});e.triggered=true;return false;}}
    ...
    if(e.type==='ironBall'){
      ...
      if(!e.hit&&e.opp){const d=Math.hypot(e.x-e.opp.cx,e.y-e.opp.cy);if(d<e.r+PW/2){e.owner.dealDamage(e.opp,e.dmg);e.opp.vx=e.vx*1.5;e.opp.vy=-10;spawnParticles(e.x,e.y,'#888',10);spawnParticles(e.x,e.y,e.owner.color,8);addFX({type:'crushRing',x:e.x,y:e.y,color:'#aaaaaa',t:450});addFX({type:'screenShake',t:200,mag:9});e.hit=true;return false;}}
    ...
    if(e.type==='massOrb'){
      ...
      e.x+=e.vx; e.y+=e.vy;
      if(e.trail){e.trail.push({x:e.x,y:e.y});if(e.trail.length>10)e.trail.shift();}
      if(!e.hit&&e.opp){
        const d=Math.hypot(e.x-e.opp.cx,e.y-e.opp.cy);
        if(d<e.r+PW/2){
          e.owner.dealDamage(e.opp,e.dmg);
          if(e.ultStun)e.opp.stunned=Math.max(e.opp.stunned,500);
          e.opp.vx=e.vx*2.8;e.opp.vy=-14;
          spawnParticles(e.x,e.y,e.owner.color,15);spawnParticles(e.x,e.y,'#000',6);
    ...
    // Soul Isomer — Mahito's transfigured human projectile, absorbs 1 hit for owner
    if(e.type==='soulIsomer'){
      if(e.trail){e.trail.push({x:e.x,y:e.y});if(e.trail.length>8)e.trail.shift();}
      if(e.tracking&&e.trackTimer>0&&e.opp){
        const dx=e.opp.cx-e.x,dy=e.opp.cy-e.y,d=Math.sqrt(dx*dx+dy*dy);
        if(d>0){e.vx+=dx/d*0.8;e.vy+=dy/d*0.5;}
        e.trackTimer-=dt;
      }
      e.x+=e.vx;e.y+=e.vy;e.vy+=0.15;
      const _ispd=Math.hypot(e.vx,e.vy);if(_ispd>14){e.vx=e.vx/_ispd*14;e.vy=e.vy/_ispd*14;}
      if(!e.hit&&e.opp){
        const d=Math.hypot(e.x-e.opp.cx,e.y-e.opp.cy);
        if(d<PW*0.8){
          e.owner.dealDamage(e.opp,e.dmg);
          spawnParticles(e.x,e.y,e.color,20);spawnParticles(e.x,e.y,'#fff',8);
          addFX({type:'burstRing',x:e.x,y:e.y,color:e.color,t:400,r:55});
          addFX({type:'bigText',x:e.opp.cx,y:e.opp.y-40,text:'SOUL CRUSH',color:e.color,t:800});
          e.hit=true;return false;
        }
      }
      if(e.x<-80||e.x>W+80||e.y>H+80)return false;
    }
    // Resonance Orb — cursed nail lob that marks on hit
    if(e.type==='resonanceOrb'){
      // Homing toward opponent while trackTimer active
      if(e.tracking&&e.trackTimer>0&&e.opp){
        const dx=e.opp.cx-e.x,dy=e.opp.cy-e.y,d=Math.sqrt(dx*dx+dy*dy);
        if(d>0){e.vx+=dx/d*1.4;e.vy+=dy/d*1.1;}
        e.trackTimer-=dt;
      } else {
        e.vy+=0.18; // gentle arc after tracking ends
      }
      // Speed cap so it doesn't overshoot wildly
      const ospd=Math.sqrt(e.vx*e.vx+e.vy*e.vy);
      if(ospd>18){e.vx=e.vx/ospd*18;e.vy=e.vy/ospd*18;}
      e.x+=e.vx; e.y+=e.vy;
      if(e.trail){e.trail.push({x:e.x,y:e.y});if(e.trail.length>8)e.trail.shift();}
      if(!e.hit&&e.opp){
        const d=Math.hypot(e.x-e.opp.cx,e.y-e.opp.cy);
        if(d<PW*0.85){
          e.owner.dealDamage(e.opp,e.dmg);
          e.opp.strawDollMarked=true;e.opp.strawDollMarkTimer=12000;
          spawnParticles(e.x,e.y,e.color,28);spawnParticles(e.x,e.y,'#ffddaa',14);
          addFX({type:'burstRing',x:e.opp.cx,y:e.opp.cy,color:e.color,t:420,r:52});
          addFX({type:'burstRing',x:e.opp.cx,y:e.opp.cy,color:'#ffddaa',t:380,r:28});
          addFX({type:'bigText',x:e.opp.cx,y:e.opp.y-54,text:'⛩ RESONANCE — MARKED',color:e.color,t:1100});
          addFX({type:'screenFlash',color:e.color,t:180,alpha:0.26,dur:180});
          addFX({type:'screenShake',t:160,mag:7});
          e.hit=true;return false;
        }
      }
      if(e.x<-60||e.x>W+60||e.y>H+60)return false;
      ...
      if(!e.active){
        ...
        if(e.opp){
          ...
          if(d<90){
            ...
            spawnParticles(e.x,e.y,e.owner?e.owner.color:'#ff44cc',15);
        ...
        // BLACK HOLE ACTIVE — enemy gets crushed; user is NOT pulled (immune to gravity) but still takes self-damage
        if(p1&&p2){
          [p1,p2].forEach(pl=>{
            ...
            if(!isUser){
              // Enemy: EXTREME pull — nearly inescapable, overwhelms all movement
              const dotProduct=pl.vx*(-nx)+pl.vy*(-ny);
              const forceScale=1.0;
              const baseForce=55/(d*0.008+0.4);
              pl.vx+=nx*baseForce*forceScale;
              pl.vy+=ny*baseForce*0.75*forceScale;
              const spd=Math.sqrt(pl.vx*pl.vx+pl.vy*pl.vy);
              if(spd>60){pl.vx=pl.vx/spd*60;pl.vy=pl.vy/spd*60;}
            }
            // Damage in 180px radius — 30/s for BOTH user and enemy
            if(d<180&&pl.hp>0){
              const dmgRate=30;
          ...
          if(e.lastDmgTxt<=0){
            ...
            [p1,p2].forEach(pl=>{
              ...
              if(d2<180&&pl.hp>0){
                const isUser=pl===e.owner;
                addFX({type:'dmgNum',x:pl.cx,y:pl.y-5,val:30,color:e.owner?e.owner.color:'#ff44cc',t:900});
              }
            });
          }
        }
...
function drawFX(){
  ...
  effects.forEach(e=>{
    ...
    if(e.type==='gamblerBg'&&e.owner){
      ...
      // Overhead fluorescent ceiling panels — skip when barely visible
      ctx.globalAlpha=fade*0.55;
      if(fade>0.15)for(let i=0;i<8;i++){
      ...
      for(let i=0;i<6;i++){const f=i/5,y2=vp.y+(H-vp.y)*f,wid=(120+180*f);ctx.beginPath();ctx.moveTo(vp.x-wid,y2);ctx.lineTo(vp.x+wid,y2);ctx.stroke();}
      ...
      if(e.owner&&e.owner.jackpotDigits){
        ...
        ctx.shadowBlur=0;
        // Next spin timer bar — uses owner's live interval value
        const spinInterval=e.owner._jackpotSlotInterval||2500;
        const barW=180,barFill=barW*(1-Math.max(0,e.owner.jackpotSlotTimer)/spinInterval);
        ...
        // Interval label (shows speed buff)
        if(spinInterval<2500){ctx.globalAlpha=fade*0.85;ctx.fillStyle='#44ffcc';ctx.font='bold 9px "Courier New"';ctx.textAlign='center';ctx.fillText('⚡ SPEED ROLL',boardX+100,boardY+96);}
      }

      // ── Big jackpot digit reveal — shown for a few seconds after winning ──
      // (jackpotBigNums FX handles this separately — see below)
    ...
    // SELF-EMBODIMENT OF PERFECTION — Mahito's soul domain
    if(e.type==='mahitoBg'&&e.owner){
      ctx.save();
      const t2=Date.now()*0.001;
      const fade=Math.min(1,(8000-e.t)/500);
      ctx.globalAlpha=fade;
      // Grey void
      const sky=ctx.createLinearGradient(0,0,0,H);
      sky.addColorStop(0,'#06060e');sky.addColorStop(0.5,'#0a0a14');sky.addColorStop(1,'#050508');
      ctx.fillStyle=sky;ctx.fillRect(0,0,W,H);
      // Distorted human silhouettes in background
      ctx.globalAlpha=fade*0.32;
      for(let i=0;i<8;i++){
        const hx=100+i*(W-200)/7+Math.sin(t2*0.5+i*1.3)*28;
        const hy=80+Math.cos(t2*0.3+i)*22;
        ctx.save();ctx.translate(hx,hy);ctx.scale(0.6+Math.sin(t2*0.4+i*0.8)*0.22,1+Math.sin(t2*0.55+i)*0.35);
        ctx.fillStyle=`hsl(${270+i*4},18%,${12+i*2}%)`;
        ctx.fillRect(-11,0,22,48);
        ctx.beginPath();ctx.arc(0,-15,11,0,Math.PI*2);ctx.fill();
        ctx.strokeStyle=ctx.fillStyle;ctx.lineWidth=5;ctx.lineCap='round';
        ctx.beginPath();ctx.moveTo(-11,10);ctx.lineTo(-26+Math.sin(t2+i)*14,34);ctx.stroke();
        ctx.beginPath();ctx.moveTo(11,10);ctx.lineTo(26+Math.cos(t2+i)*12,32);ctx.stroke();
        ctx.restore();
      }
      // Purple soul grid
      ctx.globalAlpha=fade*(0.14+0.06*Math.sin(t2*2));
      ctx.strokeStyle='#8833cc';ctx.lineWidth=0.8;
      for(let i=0;i<12;i++){const lx=i*W/11;ctx.beginPath();ctx.moveTo(lx,0);ctx.lineTo(lx+Math.sin(t2*0.5+i)*38,H);ctx.stroke();}
      for(let i=0;i<7;i++){const ly=i*H/6;ctx.beginPath();ctx.moveTo(0,ly+Math.sin(t2*0.4+i)*18);ctx.lineTo(W,ly+Math.cos(t2*0.4+i)*18);ctx.stroke();}
      // Floating soul orbs
      ctx.globalAlpha=fade*(0.38+0.18*Math.sin(t2*3));
      for(let i=0;i<6;i++){
        const ox=W*(0.1+i*0.16)+Math.sin(t2*0.6+i)*55,oy=75+Math.cos(t2*0.4+i*1.1)*45+i*38;
        const og=ctx.createRadialGradient(ox,oy,0,ox,oy,20);
        og.addColorStop(0,'#cc88ff');og.addColorStop(0.5,'#6622aa');og.addColorStop(1,'rgba(0,0,0,0)');
        ctx.fillStyle=og;ctx.fillRect(ox-22,oy-22,44,44);
      }
      // TRUE DAMAGE side labels
      ctx.globalAlpha=fade*(0.22+0.18*Math.sin(t2*5));
      ctx.save();ctx.shadowColor='#aa55ee';ctx.shadowBlur=10;
      txt('TRUE DAMAGE',W*0.12,H/2,11,'#aa55ee');
      txt('TRUE DAMAGE',W*0.88,H/2,11,'#aa55ee');
      ctx.restore();
      // Dark vignette
      ctx.globalAlpha=fade*0.5;
      const vig=ctx.createRadialGradient(W/2,H/2,H*0.2,W/2,H/2,W*0.85);
      vig.addColorStop(0,'rgba(0,0,0,0)');vig.addColorStop(1,'rgba(4,0,12,0.95)');
      ctx.fillStyle=vig;ctx.fillRect(0,0,W,H);
      ctx.globalAlpha=1;ctx.restore();
    }
  });

  // ── BLACK HOLE ORB: dedicated pass drawn AFTER all domain bgs so it
  // always renders on top regardless of where it sits in the effects array ──
  // Handles BOTH inactive (floating orb) and active (singularity) states here.
  effects.forEach(e=>{
    if(e.type!=='blackHoleOrb')return;
    const col2=e.owner?e.owner.color:'#ff44cc';
    const t2=Date.now()*0.001;
    if(!e.active){
      // ── INACTIVE: ominous floating orb — bright glow so visible against any domain bg ──
      const pulse=0.75+0.25*Math.sin(t2*12);
      ctx.save();
      ctx.globalAlpha=pulse;
      const vg2=ctx.createRadialGradient(e.x,e.y,0,e.x,e.y,28);
      vg2.addColorStop(0,'#000000');vg2.addColorStop(0.6,'#000000');vg2.addColorStop(1,col2);
      ctx.fillStyle=vg2;ctx.shadowColor=col2;ctx.shadowBlur=28;
      ctx.beginPath();ctx.arc(e.x,e.y,28,0,Math.PI*2);ctx.fill();
      // Bright contrasting outer ring (always visible against dark or bright bgs)
      ctx.strokeStyle='#ffffff';ctx.lineWidth=2;ctx.shadowColor='#ffffff';ctx.shadowBlur=14;
      ctx.beginPath();ctx.arc(e.x,e.y,32+Math.sin(t2*9)*3,0,Math.PI*2);ctx.stroke();
      ctx.strokeStyle=col2;ctx.lineWidth=1.5;ctx.shadowColor=col2;ctx.shadowBlur=20;
      ctx.beginPath();ctx.arc(e.x,e.y,36+Math.sin(t2*7)*2,0,Math.PI*2);ctx.stroke();
      // Pulsing danger ring
      ctx.globalAlpha=0.28*pulse;ctx.strokeStyle=col2;ctx.lineWidth=1.5;ctx.setLineDash([4,5]);
      ctx.shadowBlur=0;ctx.beginPath();ctx.arc(e.x,e.y,90,0,Math.PI*2);ctx.stroke();ctx.setLineDash([]);
      // Warning pulse
      if(Math.sin(t2*6)>0.2){ctx.globalAlpha=Math.max(0,(Math.sin(t2*6)-0.2)*0.85);txt('⚠ ACTIVATE',e.x,e.y-46,9,col2);}
      ctx.globalAlpha=1;ctx.shadowBlur=0;ctx.restore();
      return;
    }
      ctx.save();
  ...
  effects.forEach(e=>{
    ...
    switch(e.type){
      ...
      case'staticRushTrail':{const a=Math.max(0,e.t/500);ctx.globalAlpha=a*0.8;ctx.shadowColor=e.color;ctx.shadowBlur=10;for(let i=0;i<8;i++){const xx=e.x1+(e.x2-e.x1)*(i/7);ctx.strokeStyle=`hsl(${190+i*8},100%,${55+i*4}%)`;ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(xx,e.y-25);ctx.lineTo(xx+(Math.random()-.5)*22,e.y+55);ctx.stroke();}break;}
      ...
      case'bolt':{if(e.trail&&e.trail.length>1){ctx.shadowColor=e.color;ctx.shadowBlur=10;for(let i=1;i<e.trail.length;i++){const ta=i/e.trail.length;ctx.globalAlpha=ta*0.6;ctx.strokeStyle=e.glow||e.color;ctx.lineWidth=e.h*ta;ctx.beginPath();ctx.moveTo(e.trail[i-1].x,e.trail[i-1].y);ctx.lineTo(e.trail[i].x,e.trail[i].y);ctx.stroke();}}ctx.globalAlpha=1;ctx.fillStyle=e.color;ctx.shadowColor=e.color;ctx.shadowBlur=15;ctx.beginPath();ctx.ellipse(e.x,e.y,e.w/2,e.h/2,Math.atan2(e.vy,e.vx),0,Math.PI*2);ctx.fill();break;}
      ...
      case'godslayer':{
        // Only render once hit is confirmed (e.triggered set by cc_player.js on hit)
        if(!e.triggered)break;
        const a=Math.max(0,e.t/700),lw=80*(0.5+a*0.5);
        ctx.globalAlpha=a*0.7;
        const g=ctx.createLinearGradient(e.x,0,e.x,FLOOR);
        g.addColorStop(0,'rgba(0,0,0,0)');g.addColorStop(0.3,'#aa44ff');g.addColorStop(0.7,'#ffffff');g.addColorStop(1,'#aa44ff');
        ctx.fillStyle=g;ctx.fillRect(e.x-lw/2,0,lw,FLOOR);
        ctx.globalAlpha=a;ctx.strokeStyle='#ffffff';ctx.lineWidth=4;ctx.shadowColor='#cc44ff';ctx.shadowBlur=20;
        ctx.beginPath();let gx=e.x;ctx.moveTo(gx,0);
        for(let gy=0;gy<FLOOR;gy+=30){gx=e.x+(Math.random()-.5)*40;ctx.lineTo(gx,gy);}
        ctx.lineTo(e.x,FLOOR);ctx.stroke();break;}
      ...
      case'shrineSlashStorm':{
        ...
        // Diagonal slashes — shadowBlur set ONCE, not per stroke
        ctx.shadowColor=e.owner.color;ctx.shadowBlur=14;
        const numSlash=4;
        for(let i=0;i<numSlash;i++){
          ...
          const len=110+Math.sin(phase*1.3+i)*50;
          ctx.globalAlpha=Math.max(0,a*(0.4+0.3*Math.sin(phase)));
          ctx.strokeStyle=i%2===0?e.owner.color:e.owner.def.accent;
          ctx.lineWidth=3;
          ctx.beginPath();
        ...
        if(opp2){
          ...
          ctx.globalAlpha=xA;
          ctx.strokeStyle=e.owner.color;ctx.lineWidth=5;ctx.shadowBlur=18;
          ctx.beginPath();ctx.moveTo(tx-xLen,ty-xLen);ctx.lineTo(tx+xLen,ty+xLen);ctx.stroke();
          ctx.beginPath();ctx.moveTo(tx+xLen,ty-xLen);ctx.lineTo(tx-xLen,ty+xLen);ctx.stroke();
        }
        // Sweep lines — NO shadow, cheap
        ctx.shadowBlur=0;ctx.globalAlpha=a*0.07;
        ctx.strokeStyle='#cc1100';ctx.lineWidth=1;
        for(let i=0;i<6;i++){
          const sweep=(t2*90+i*200)%1400-200;
      ...
      case'jackpotWin':{
        ...
        break;}
      case'jackpotBigNums':{
        // Giant winning digits shown in the background for dramatic effect
        if(!e.digits)break;
        const lifeMax=3800,fadeIn=500,fadeOut=900;
        const elapsed=lifeMax-e.t;
        const fadeA=elapsed<fadeIn?(elapsed/fadeIn):e.t<fadeOut?(e.t/fadeOut):1;
        ctx.save();
        // Subtle dark backing
        ctx.globalAlpha=fadeA*0.28;
        ctx.fillStyle='#000a05';
        ctx.fillRect(0,0,W,H);
        // The three big digits
        const digits=e.digits;
        const isOdd=digits[0]%2!==0;
        const numCol=isOdd?'#ffaa44':'#44ffdd';
        const glowCol=isOdd?'#ff6600':'#00ffaa';
        ctx.font=`bold ${Math.floor(H*0.52)}px "Courier New"`;
        ctx.textAlign='center';
        ctx.shadowColor=glowCol;ctx.shadowBlur=80;
        ctx.globalAlpha=fadeA*0.12;
        ctx.fillStyle=glowCol;
        // Draw once for wide glow
        ctx.fillText(digits.join(''),W/2,H*0.72);
        // Draw crisp on top
        ctx.shadowBlur=30;ctx.globalAlpha=fadeA*0.22;
        ctx.fillStyle=numCol;
        ctx.fillText(digits.join(''),W/2,H*0.72);
        // Parity label underneath
        ctx.font=`bold 22px "Courier New"`;
        ctx.shadowBlur=14;ctx.globalAlpha=fadeA*0.55;
        ctx.fillStyle=numCol;
        ctx.fillText(isOdd?'ODD  →  NEXT DOMAIN: +40%':'EVEN  →  NEXT DOMAIN: 1.5s ROLL',W/2,H*0.8);
        ctx.globalAlpha=1;ctx.shadowBlur=0;ctx.restore();
        break;}
      ...
      case'maxOutputAura':{if(!e.owner)break;const t2=Date.now()*0.001,p=0.3+0.25*Math.sin(t2*8);ctx.globalAlpha=p;const mc=e.color||e.owner.def.glow;ctx.strokeStyle=mc;ctx.lineWidth=5;ctx.shadowColor=mc;ctx.shadowBlur=24;ctx.strokeRect(e.owner.x-16,e.owner.y-16,PW+32,PH+32);ctx.beginPath();for(let i=0;i<6;i++){const ang=i*Math.PI/3+t2*2,r1=48,r2=70+Math.sin(t2*5+i)*12;ctx.moveTo(e.owner.cx+Math.cos(ang)*r1,e.owner.cy+Math.sin(ang)*r1);ctx.lineTo(e.owner.cx+Math.cos(ang)*r2,e.owner.cy+Math.sin(ang)*r2);}ctx.stroke();break;}
      ...
      case'stagnationAura':{if(!e.owner||!e.owner.stagnationReady)break;const a=0.3+0.2*Math.sin(Date.now()*0.015);ctx.globalAlpha=a;ctx.strokeStyle='#aabbff';ctx.lineWidth=1.5;ctx.strokeRect(e.owner.x-8,e.owner.y-8,PW+16,PH+16);break;}
      ...
      case'resonanceOrb':{
        // Cursed orb with orange glow trail
        if(e.trail&&e.trail.length>1){
          ctx.shadowColor=e.color;ctx.shadowBlur=8;
          for(let i=1;i<e.trail.length;i++){
            const ta=i/e.trail.length;
            ctx.globalAlpha=ta*0.5;ctx.strokeStyle=e.color;ctx.lineWidth=3*ta;
            ctx.beginPath();ctx.moveTo(e.trail[i-1].x,e.trail[i-1].y);ctx.lineTo(e.trail[i].x,e.trail[i].y);ctx.stroke();
          }
        }
        ctx.globalAlpha=1;ctx.shadowColor=e.color;ctx.shadowBlur=20;
        const t2=Date.now()*0.006;
        const rg=ctx.createRadialGradient(e.x,e.y,0,e.x,e.y,14);
        rg.addColorStop(0,'#ffffff');rg.addColorStop(0.4,e.color);rg.addColorStop(1,'rgba(0,0,0,0)');
        ctx.fillStyle=rg;ctx.beginPath();ctx.arc(e.x,e.y,14,0,Math.PI*2);ctx.fill();
        // Rotating spike ring
        ctx.lineWidth=2;ctx.strokeStyle=e.color;
        for(let i=0;i<6;i++){const a=i/6*Math.PI*2+t2*3;ctx.globalAlpha=0.7;ctx.beginPath();ctx.moveTo(e.x+Math.cos(a)*9,e.y+Math.sin(a)*9);ctx.lineTo(e.x+Math.cos(a)*18,e.y+Math.sin(a)*18);ctx.stroke();}
        ctx.globalAlpha=1;ctx.shadowBlur=0;break;}
      case'soulIsomer':{
        if(e.trail&&e.trail.length>1){
          for(let i=1;i<e.trail.length;i++){const ta=i/e.trail.length;ctx.globalAlpha=ta*0.22;ctx.fillStyle=e.color;ctx.beginPath();ctx.arc(e.trail[i].x,e.trail[i].y,7*ta,0,Math.PI*2);ctx.fill();}
        }
        const t2=Date.now()*0.001;
        ctx.globalAlpha=0.85+0.15*Math.sin(t2*8);
        ctx.strokeStyle=e.color;ctx.lineWidth=2;ctx.shadowColor=e.color;ctx.shadowBlur=18;
        // Head
        ctx.beginPath();ctx.arc(e.x,e.y-16,10,0,Math.PI*2);ctx.stroke();
        // Torso
        ctx.beginPath();ctx.moveTo(e.x,e.y-6);ctx.lineTo(e.x,e.y+12);ctx.stroke();
        // Warped arms
        ctx.beginPath();
        ctx.moveTo(e.x,e.y-2);ctx.lineTo(e.x+14+Math.sin(t2*6)*8,e.y+5);
        ctx.moveTo(e.x,e.y-2);ctx.lineTo(e.x-14-Math.sin(t2*6)*8,e.y+5);
        ctx.stroke();
        // Warped legs
        ctx.beginPath();
        ctx.moveTo(e.x,e.y+12);ctx.lineTo(e.x+9,e.y+26+Math.sin(t2*5)*5);
        ctx.moveTo(e.x,e.y+12);ctx.lineTo(e.x-9,e.y+26+Math.cos(t2*5)*5);
        ctx.stroke();
        // Soul glow
        ctx.globalAlpha=0.38+0.2*Math.sin(t2*4);
        const sg=ctx.createRadialGradient(e.x,e.y,0,e.x,e.y,28);
        sg.addColorStop(0,e.color);sg.addColorStop(1,'rgba(0,0,0,0)');
        ctx.fillStyle=sg;ctx.beginPath();ctx.arc(e.x,e.y,28,0,Math.PI*2);ctx.fill();
        ctx.shadowBlur=0;break;}
      case'massOrb':{        if(e.trail&&e.trail.length>1){
        ...
        ctx.globalAlpha=1;ctx.shadowBlur=0;break;}
      // blackHoleOrb: all rendering handled by dedicated pass above drawFX (always on top of domain bgs)
      case'resonanceOrb':{
        // Cursed orb with orange glow trail
        if(e.trail&&e.trail.length>1){
          ctx.shadowColor=e.color;ctx.shadowBlur=8;
  ...
  // ── Batch particle draw (no per-particle save/shadowBlur — big perf win) ──
  if(particles.length){
    const _pm={};
    for(const p of particles){(_pm[p.color]||(_pm[p.color]=[])).push(p);}
    for(const [col,ps] of Object.entries(_pm)){
      ctx.fillStyle=col;
      for(const p of ps){
