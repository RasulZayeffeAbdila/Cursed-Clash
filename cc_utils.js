...
// Wrap text to fit maxWidth — returns array of line strings
function wrapText(str,maxW,size,bold=false){
...
// ── BUTTON — hover-SFX tracker keyed by position ──
const _btnHov={};
function btn(x,y,w,h,label,color='#aa66ff',subLabel=''){
  const _hKey=x+','+y;
  const hov=isHov(x,y,w,h);
  if(hov&&!_btnHov[_hKey]&&typeof SFX!=='undefined')SFX.hover();
  _btnHov[_hKey]=hov;
