...
function startRound(){
  ...
  if(gameMode==='bindedbattle'){
    ...
    // Discharged: start with full ult bar (ready domain)
    if(p1Vow==='discharged')p1.ult=MAX_ULT;
    if(p2Vow==='discharged')p2.ult=MAX_ULT;
    // Overwhelming: start with full ult bar so player can activate and win clash
    if(p1Vow==='overwhelming')p1.ult=MAX_ULT;
    if(p2Vow==='overwhelming')p2.ult=MAX_ULT;
    // Cursed Vitality: +250hp
    ...
    // Domain Vow: -25% starting HP
    if(p1Vow==='domainVow'){p1.hp=Math.max(1,Math.round(p1.hp*0.75));p1.maxHp=p1.hp;}
    if(p2Vow==='domainVow'){p2.hp=Math.max(1,Math.round(p2.hp*0.75));p2.maxHp=p2.hp;}
    setTimeout(()=>{
...
function startTraining(){
  trainingMode=true;
  gameMode='training';
  roundNum=1;p1RoundWins=0;p2RoundWins=0;matchRoundsData=[];
  const same=p1Idx===p2Idx;
  p1=new Player(CHAR_NAMES[p1Idx],200,1,1,false);
  p2=new Player(CHAR_NAMES[p2Idx],W-200-PW,-1,2,same);
  p1.maxHp=999;p1.hp=999; // P1 also very durable in training
  effects=[];particles=[];pendingDomainP1=null;pendingDomainP2=null;domainClash=null;
  countdownTimer=2000;state='countdown';
}

function resetTraining(){
  if(!p1||!p2)return;
  p1.hp=p1.maxHp;p1.x=200;p1.y=FLOOR-PH;p1.vx=0;p1.vy=0;p1.stunned=0;p1.cooldowns=[0,0,0,0];p1.ult=0;p1.ultActive=false;p1.ultTimer=0;p1.domainActive=false;p1.blackHoleOrb=null;
  p2.hp=p2.maxHp;p2.x=W-200-PW;p2.y=FLOOR-PH;p2.vx=0;p2.vy=0;p2.stunned=0;p2.cooldowns=[0,0,0,0];p2.ult=0;p2.ultActive=false;p2.ultTimer=0;p2.domainActive=false;p2.blackHoleOrb=null;
  effects=[];particles=[];domainClash=null;pendingDomainP1=null;pendingDomainP2=null;
  spawnParticles(W/2,H/2,'#aaaaff',25);
  addFX({type:'bigText',x:W/2,y:H/2-40,text:'SESSION RESET',color:'#aaaaff',t:1000});
}
